...
기존에 두 문장을 비교하기 위해 fully self-attention을 사용하는 방식은 성능에 비해 큰 비용을 요구합니다.
`Poly-encoder`는 `Bi-encoder`와 `Cross-encoder`를 적절하게 사용하여 낮은 비용으로 추론을 하도록 구성된 모델입니다.
...
<Comment>
Retriever 모듈에서 context와 candidate의 cross-attention 연산을 통해 생성된 $y_{ctxt}$와 $y_{cand}$를 dot product 하여 score를 구합니다.
그리고 context와 가장 높은 점수를 얻은 candidate를 tokenize 하여 이를 Decoder 모듈의 입력으로 사용하는 방식이죠.
그런데 왜 이미 context와 candidate의 정보를 모두 이용한 $y_{ctxt}$를 입력으로 사용하지 않을까요?

생각해보니 BlenderBot은 end-to-end 모델이 아닌 각 모듈들을 각자 다른 방식으로 fine-tuning을 진행하기 떄문에 그런듯.
</Comment>

<Comment>
Retriever 모듈에서 context(input) - response(label) pair를 사용하여 학습을 진행하였는데
...
이를 앞서 사용한 `Retriever`와 동일한 모델을 이용하여 순위를 매기고 단일 문장을 선택합니다.
