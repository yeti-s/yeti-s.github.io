...
1\. <ColorText color='var(--error)'>단어의 순서를 고려하지 못합니다.</ColorText>
...
그래서 이를 해결하기 위해 <ColorText color='var(--info)'>위치에 관한 정보를 embedding 해줍니다.</ColorText>
...
* <ColorText color='var(--info)'>위치 정보가 절대적이지 않고 주기성을 가져 모델이 위치 정보를 유연하게 해석합니다.</ColorText>
* <ColorText color='var(--info)'>주기적인 패턴 덕분에 모델이 훈련 중 본 적 없는 더 긴 시퀀스에 대해 추론할 수 있습니다.</ColorText>
* <ColorText color='var(--error)'>그러나 위치 표현이 학습 가능하지 않고 실제로 긴 시퀀스에 대한 효과가 제한적일 수 있습니다.</ColorText>

2\. <ColorText color='var(--error)'>비선형적인 결과를 만들어 낼 수 없어요.</ColorText>
가중 합을 사용하기 때문에 self-attention 만으로는 비선형적인 결과를 만들어 낼 수 없어요.
이는 <ColorText color='var(--info)'>self-attention 결과에 feed-forward network를 적용하여 해결할 수 있습니다.</ColorText>

3\. <ColorText color='var(--error)'>문장을 예측할 때 미래를 보지 않아야 합니다.</ColorText>
self-attention은 문장의 모든 단어를 보고 연산을 진행하기 때문에 예측 시점에서 미래의 단어까지 정보로 들어올 수 있어요.
이는 <ColorText color='var(--info)'>미래에 대한 정보를 masking 하는 기법을 사용하여 해결할 수 있어요.</ColorText>

![](assets/attention/04.png)
