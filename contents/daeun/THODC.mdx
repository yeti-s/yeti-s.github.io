...
논문에서는 새로운 평가 지표인 `SSA`와 perplexity의 상관관계를 설명하고 있어요.
더 낮은 perplexity 가진 모델이 더 사람다운 응답을 만들어 낸다는 의미입니다.
또한 데이터셋을 만들기 위한 전략을 보여주고 decoding 방법까지 제안하고 있습니다.

저는 한국어를 기반으로 한 챗봇을 만들기 위해 한국어 기반 데이터셋을 구축해야 하는데 큰 틀은 여기서 제시한 방법을 활용할 수 있을 것 같습니다.
물론 자세한 방법은 약간씩 조정할 필요가 있겠지만요.
또 이 논문에서 제시한 decoding 방법을 사용하는 것도 좋은 결과를 만들것 같습니다.
...
여기서 가장 첫 메세지를 root, 이후 응답들을 각 항목의 child nodes로 구성할 수 있습니다.
...
추가로 메모리를 절약하기 위해 Adafactor opimizer로 0.01 의 lr로 시작하여 10K step동안 유지하고,
그 후부터 step의 역수의 제곱근으로 감소시켰습니다.
...
Sample-and-rank는 먼저 랜덤 샘플링을 이용하여 N개의 독립적인 후보 응답들을 생성합니다.
그 다음 확률이 가장 높은 응답을 선택하는 방식이죠.
이때 temperature T가 큰 값이면 다양하지만 상황에 적합하지 않는 단어를 만들어내고, 작은 값이면 안전하지만 흔한 단어만을 생성하는 것을 볼 수 있었습니다.

![Table 2](assets/THODC/00.png)
![Table 3](assets/THODC/01.png)
...
우리는 만약 두 차례의 메세지가 공통된 긴 sub-sequences로 이루어져 있다면 이를 후보에서 제외하는 방식의 규칙을 만들었습니다.
